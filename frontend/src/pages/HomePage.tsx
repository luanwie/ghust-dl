...
function StatusPill() {
  const [s, setS] = useState("Sistema pronto")
  useEffect(() => {
    getStatus().then(d => { if (d?.cache?.total_files) setS(`${d.cache.total_files} músicas em cache`) }).catch(() => {})
  }, [])
  return (
    <div className="inline-flex items-center gap-2 px-3.5 py-2 rounded-full bg-white/[0.04] border border-white/[0.06]">
      ...
      <span className="text-[13px] text-text-secondary font-medium">{s}</span>
    </div>
  )
}
...
export default function HomePage() {
  ...
  return (
    <>
      ...
      {/* ── Hero ── */}
      <section className="min-h-[80dvh] flex flex-col justify-center py-16">
        <Reveal><div className="flex justify-center mb-5"><StatusPill /></div></Reveal>
        <Reveal delay={0.08}>
          <h1 className="text-[2rem] sm:text-[2.75rem] lg:text-[3.5rem] font-bold text-text-primary tracking-tight leading-[1.08] text-center mb-3 text-balance">
            Música pra DJ<br /><span className="text-brand-gold">em qualidade real</span>
          </h1>
        </Reveal>
        <Reveal delay={0.15}>
          <p className="text-text-secondary text-[16px] sm:text-[17px] leading-relaxed text-center max-w-md mx-auto mb-6">
            FLAC e MP3 320kbps verdadeiros. Cache inteligente. Gratuito com contribuição voluntária.
          </p>
        </Reveal>
        <Reveal delay={0.22}><SearchBar onSearch={handleSearch} loading={loading} /></Reveal>
        <Reveal delay={0.28}>
          <div className="flex flex-wrap justify-center gap-x-5 gap-y-2.5 mt-4 text-[13px] text-text-muted">
            <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500/70" /> FLAC</span>
            <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-brand-gold/70" /> MP3 320</span>
            <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-text-muted/70" /> Cache</span>
            <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-text-muted/70" /> <Heart size={11} className="inline" /> Pix</span>
          </div>
        </Reveal>
      </section>
        ...
        <section className="pb-16">
          <div className="max-w-xl mx-auto">
            ...
            {!loading && !error && results.length === 0 && searched && query && (
              <div className="text-center py-10"><p className="text-text-muted text-[14px]">Nenhum resultado para <span className="text-text-secondary">&ldquo;{query}&rdquo;</span></p></div>
            )}
            {!loading && results.length > 0 && (
              <>
                <p className="text-text-muted text-[13px] mb-3">{results.length} resultado{results.length !== 1 ? "s" : ""}</p>
                <div className="space-y-2.5">{results.map((r, i) => <ResultCard key={r.id || i} result={r} />)}</div>
              </>
            )}
          </div>
        </section>
      ...
      {!searched && (
        <>
          ...
          <section className="py-16 sm:py-20">
            <Reveal>
              <div className="text-center mb-8 sm:mb-10">
                <p className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-brand-gold/8 border border-brand-gold/15 text-brand-gold text-[12px] font-medium mb-3">Como funciona</p>
                <h2 className="text-[1.5rem] sm:text-[2rem] font-bold text-text-primary tracking-tight">Busca. Baixa. Toca.</h2>
              </div>
            </Reveal>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              {STEPS.map((s, i) => (
                <div key={s.title} className="glass rounded-xl p-4 sm:p-5 gold-glow">
                  ...
                  <h3 className="text-text-primary font-semibold text-[16px] mt-1.5 mb-1">{s.title}</h3>
          ...
          <section className="py-16 sm:py-20 border-t border-border-subtle">
            <Reveal>
              <div className="text-center mb-8 sm:mb-10">
                <h2 className="text-[1.5rem] sm:text-[2rem] font-bold text-text-primary tracking-tight">Por que usar?</h2>
              </div>
            </Reveal>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              {FEATURES.map((f, i) => (
                <div key={f.title} className="glass rounded-xl p-4 gold-glow">
                  ...
                  <h3 className="text-text-primary font-semibold text-[16px] mb-1">{f.title}</h3>
          ...
          <section className="py-16 sm:py-20">
            <Reveal>
              <div className="max-w-lg mx-auto glass rounded-2xl p-6 sm:p-8 text-center border-brand-gold/10">
                ...
                <h2 className="text-[1.5rem] font-bold text-text-primary tracking-tight mb-2">Gostou? Apoie</h2>
                <p className="text-text-muted text-[14px] mb-5 max-w-xs mx-auto leading-relaxed">Servidor, cache, domínio. Qualquer contribuição ajuda a manter o projeto online.</p>
                <Link to="/donate" className="btn-gold text-[15px] px-8 gap-2">
                  <Heart size={18} /> Fazer um Pix
                </Link>
              </div>
            </Reveal>
          </section>
